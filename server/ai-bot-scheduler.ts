...
async function runStandaloneAIBots() {
  try {
    ...
    // Analyze existing holdings with full AI consensus BEFORE scanning for new opportunities
    console.log("[Deep Scan] Analyzing existing holdings with full AI consensus...");
    await Promise.all(whitelistedConfigs.map((c: any) => analyzeHoldingsWithFullAI(c.ownerWalletAddress)));
...
/**
 * Analyze all holdings using full 6-model AI consensus during deep scans
 * This provides high-quality position management with hivemind decision-making
 */
async function analyzeHoldingsWithFullAI(ownerWalletAddress: string) {
  try {
    console.log(`[Deep Scan Holdings] Analyzing positions for ${ownerWalletAddress.slice(0, 8)}...`);
    
    // Get all current positions
    const positions = await storage.getAIBotPositions(ownerWalletAddress);
    
    if (positions.length === 0) {
      console.log(`[Deep Scan Holdings] No positions to analyze for ${ownerWalletAddress.slice(0, 8)}...`);
      return;
    }
    
    console.log(`[Deep Scan Holdings] Found ${positions.length} positions to analyze with 6-model consensus`);
    
    // Get bot configuration
    const config = await storage.getAIBotConfig(ownerWalletAddress);
    if (!config) {
      console.log(`[Deep Scan Holdings] No config found for ${ownerWalletAddress.slice(0, 8)}...`);
      return;
    }
    
    // Get treasury keypair for potential trades
    const { loadKeypairFromPrivateKey } = await import("./solana-sdk");
    const { decrypt } = await import("./crypto");
    const treasuryPrivateKey = decrypt(
      config.treasuryKeyCiphertext!,
      config.treasuryKeyIv!,
      config.treasuryKeyAuthTag!
    );
    ...
    // Analyze each position with full AI consensus
    for (const position of positions) {
      try {
        const mint = position.tokenMint;
        console.log(`[Deep Scan Holdings] Analyzing ${position.tokenSymbol} (${mint.slice(0, 8)}...)`);
        
        // Get current price
        const currentPriceSOL = await getTokenPrice(mint);
        if (!currentPriceSOL) {
          console.log(`[Deep Scan Holdings] Cannot fetch price for ${position.tokenSymbol}`);
          continue;
        }
        ...
        console.log(`[Deep Scan Holdings] ${position.tokenSymbol}: Entry $${entryPrice.toFixed(9)} → Current $${currentPriceSOL.toFixed(9)} (${profitPercent > 0 ? '+' : ''}${profitPercent.toFixed(2)}%)`);
        
        // Use full 6-model AI consensus for position analysis
        const analysis = await analyzePositionWithFullConsensus(mint, currentPriceSOL, profitPercent);
        
        console.log(`[Deep Scan Holdings] AI Consensus: ${analysis.recommendation} (confidence: ${analysis.aiConfidence}%)`);
        console.log(`[Deep Scan Holdings] Reasoning: ${analysis.reasoning}`);
        
        // Update position with latest analysis
        await storage.updateAIBotPosition(ownerWalletAddress, mint, {
          currentPriceSOL: currentPriceSOL.toString(),
          lastCheckProfitPercent: profitPercent.toString(),
          lastAIAnalysis: analysis.reasoning,
          lastAIConfidence: analysis.aiConfidence,
        });
        
        // Act on AI recommendation
        if (analysis.recommendation === "SELL" && analysis.aiConfidence >= 50) {
          console.log(`[Deep Scan Holdings] AI recommends SELL with ${analysis.aiConfidence}% confidence - executing...`);
          
          // Execute sell
          const { swapTokensToSOL } = await import("./jupiter");
          const sellResult = await swapTokensToSOL(
            treasuryKeypair,
            mint,
            0.5 // 0.5% slippage for better execution
          );
          
          if (sellResult.success && sellResult.solReceived) {
            const solReceived = sellResult.solReceived;
            const amountInvested = parseFloat(position.amountSOL);
            const actualProfit = solReceived - amountInvested;
            const actualProfitPercent = (actualProfit / amountInvested) * 100;
            
            console.log(`[Deep Scan Holdings] ✅ Sold ${position.tokenSymbol}: ${actualProfitPercent.toFixed(2)}% profit | ${solReceived.toFixed(6)} SOL received`);
            
            // Remove position from database
            await storage.deleteAIBotPositionByMint(ownerWalletAddress, mint);
            
            // Update portfolio peak if needed
            const currentBalance = await getWalletSOLBalance(treasuryKeypair.publicKey.toString());
            if (currentBalance > parseFloat(config.portfolioPeakSOL)) {
              await storage.updateAIBotConfig(ownerWalletAddress, {
                portfolioPeakSOL: currentBalance.toString(),
              });
              console.log(`[Deep Scan Holdings] 📈 New portfolio peak: ${currentBalance.toFixed(6)} SOL`);
            }
            
          } else {
            console.log(`[Deep Scan Holdings] ❌ Failed to sell ${position.tokenSymbol}: ${sellResult.error}`);
          }
          
        } else if (analysis.recommendation === "HOLD") {
          console.log(`[Deep Scan Holdings] AI recommends HOLD - continuing to monitor`);
          
        } else if (analysis.recommendation === "ADD") {
          console.log(`[Deep Scan Holdings] AI recommends ADD MORE - position showing strong momentum`);
          // Could implement add-to-position logic here in the future
        }
        
      } catch (error) {
        console.error(`[Deep Scan Holdings] Error analyzing ${position.tokenSymbol}:`, error);
      }
    }
    
    console.log(`[Deep Scan Holdings] Completed analysis for ${ownerWalletAddress.slice(0, 8)}...`);
    
  } catch (error) {
    console.error(`[Deep Scan Holdings] Error for wallet ${ownerWalletAddress}:`, error);
  }
}

/**
 * Analyze position using full 6-model AI consensus (used during deep scans)
 * Returns comprehensive analysis with recommendation and reasoning
 */
async function analyzePositionWithFullConsensus(
  tokenMint: string,
  ...
  try {
    ...
    if (pairs.length === 0) {
      return {
        recommendation: "SELL",
        aiConfidence: 80,
        reasoning: "No market data available - position may be illiquid, recommend exiting"
      };
    }
    ...
    const prompt = `You are analyzing a currently held cryptocurrency position for position management decisions.
...
Analyze this position and provide a comprehensive recommendation:

RECOMMENDATION OPTIONS:
1. SELL - Exit the position due to:
   - Weakening momentum or reversal signals
   - Declining liquidity or volume
   - Profit target reached or risk of drawdown
   - Better opportunities elsewhere
   
2. HOLD - Maintain current position due to:
   - Strong continued momentum
   - Healthy market indicators
   - Upside potential remains strong
   
3. ADD - Consider adding to position due to:
   - Exceptional momentum acceleration
   - Strong confluence of bullish signals
   - Position showing outsized potential

CONFIDENCE (0-100):
- 80-100: Very strong conviction in recommendation
- 60-79: Strong conviction
- 40-59: Moderate conviction
- 20-39: Low conviction
- 0-19: Very uncertain

Provide:
1. RECOMMENDATION: SELL, HOLD, or ADD
2. CONFIDENCE: 0-100
3. REASONING: Detailed analysis (3-4 sentences)

Consider:
- Momentum trends (strengthening or weakening?)
- Liquidity health (sufficient for safe management?)
- Volume patterns (increasing or declining?)
...
{
  "recommendation": "<SELL, HOLD, or ADD>",
  "confidence": <number 0-100>,
  "reasoning": "<your detailed analysis>"
}`;

    // Get all AI clients for consensus voting
    const { getAllAIClients } = await import("./grok-analysis");
    ...
    console.log(`[AI Consensus] Querying ${clients.length} models for position analysis`);
        ...
        try {
          const response = await client.chat.completions.create({
            ...
            messages: [
              {
                role: "system",
                content: "You are a professional cryptocurrency trading analyst. Analyze positions and provide SELL, HOLD, or ADD recommendations with confidence and reasoning. Always respond with valid JSON."
              },
              {
                role: "user",
                content: prompt
          ...
          const recommendation = (parsed.recommendation || "HOLD").toUpperCase();
          const confidence = Math.min(100, Math.max(0, parsed.confidence || 50));
          const reasoning = parsed.reasoning || "Analysis completed";
          
          return {
            provider,
            recommendation: recommendation as "SELL" | "HOLD" | "ADD",
            confidence,
            reasoning,
            success: true
          };
        } catch (error) {
          console.error(`[AI Consensus] ${provider} failed:`, error instanceof Error ? error.message : String(error));
          return {
            provider,
    ...
    console.log(`[AI Consensus] ${successfulVotes.length}/${clients.length} models responded`);
    
    // Calculate consensus
    const sellVotes = successfulVotes.filter(v => v.recommendation === "SELL").length;
    const holdVotes = successfulVotes.filter(v => v.recommendation === "HOLD").length;
    const addVotes = successfulVotes.filter(v => v.recommendation === "ADD").length;
    
    const totalVotes = successfulVotes.length;
    const avgConfidence = successfulVotes.reduce((sum, v) => sum + v.confidence, 0) / totalVotes;
    
    // Determine consensus recommendation (majority wins)
    let recommendation: "SELL" | "HOLD" | "ADD";
    if (sellVotes > holdVotes && sellVotes > addVotes) {
      recommendation = "SELL";
    } else if (addVotes > holdVotes && addVotes > sellVotes) {
      recommendation = "ADD";
    } else {
      recommendation = "HOLD";
    }
    
    // Build comprehensive reasoning from all models
    const voteSummary = `${sellVotes} SELL, ${holdVotes} HOLD, ${addVotes} ADD`;
    const reasoningSummary = `AI Consensus (${totalVotes} models): ${voteSummary}. ` +
      successfulVotes.slice(0, 3).map(v => `${v.provider}: ${v.reasoning.slice(0, 80)}`).join("; ");
    
    console.log(`[AI Consensus] Position Analysis: ${recommendation} (${avgConfidence.toFixed(0)}% avg confidence) - ${voteSummary}`);
    
    return {
      recommendation,
      aiConfidence: Math.round(avgConfidence),
      reasoning: reasoningSummary.slice(0, 500), // Limit size
    };

  } catch (error) {
    console.error("[AI Consensus] Error analyzing position:", error);
    // Fallback to conservative approach
    return {
      recommendation: profitPercent < -30 ? "SELL" : "HOLD",
      aiConfidence: 50,
      reasoning: `Analysis error - using fallback logic based on ${profitPercent.toFixed(2)}% P/L`
    };
  }
}
